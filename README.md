...
If these claims are altered by a malicious client to attempt an elevation-of-privilege attack, the client will receive an HTTP 401 Unauthorized exception.  Why?  Because the JWT authentication handler in the Identity service (in any ASP.NET Core app) hashes the claims using a CredentialSecret string known only to the Identity service (never transmitted to the client), determines it does not match the binary hash contained in the JWT token (the scrambled characters above), and concludes the JWT token has been tampered with.

This JWT token may be used by clients to authenticate to the Identity service to to any other service that signs its JWT tokens using the same CredentialSecret string.  Obviously this token is more secure when transmitted over an encrypted HTTP channel via [SSL / TLS](https://en.wikipedia.org/wiki/Transport_Layer_Security).
