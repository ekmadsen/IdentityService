...
This JWT token may be used by clients to authenticate to the Identity service to to any other service that signs its JWT tokens using the same CredentialSecret string.  Obviously this token is more secure when the network transport layer is encrypted via [SSL / TLS.](https://en.wikipedia.org/wiki/Transport_Layer_Security).
